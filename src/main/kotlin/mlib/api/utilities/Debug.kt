...
object Debug {
    ...
    fun toggleDebug() {
        MLib.instances.forEach { (plugin, _) ->
            val currentState = enabledPlugins[plugin] ?: false
            enabledPlugins[plugin] = !currentState
            plugin.logger.log(Level.INFO, "Debugging is now ${if (!currentState) "enabled" else "disabled"} for ${plugin.name}")
        }
    }
    /**
     * Log a debug message if debugging is enabled for the plugin.
